    ...
    load() {
        ...
        this.addEdits(
            new EditFunction(CanvasDraw.prototype, "score").load(str => {
                str = plugins.insertAfter(str, 'strings.good === "濶ｯ"', ` && config.score !== "adlib"`)
            ...
            new EditValue(allStrings.ja.note, "green").load(() => "繧ｰ繝ｪ繝ｼ繝ｳ"),
            new EditValue(allStrings.cn.note, "green").load(() => "扈ｿ"),
            new EditValue(allStrings.tw.note, "green").load(() => "邯�"),
            new EditValue(allStrings.ko.note, "green").load(() => "�ｹ��"),
