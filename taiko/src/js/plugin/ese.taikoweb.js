export default class Plugin extends Patch{
	...
	loadEseSongs(){
		...
		image.src = "https://taiko-asset.kairun.jp/img/bg_genre_kids.png" + `?${ts}`
		...
		var removedSongs = assets.songsDefault
		
		// Removes non-creative songs from the server songlist
		this.addEdits(
			new EditValue(assets, "songsDefault").load(() => assets.songsDefault),
			new EditValue(assets, "songs").load(() => assets.songsDefault)
		)

		promises.push(loader.ajax("https://taiko-asset.kairun.jp/musicinfo.json" + `?${ts}`).then(songs => {
			songs = JSON.parse(songs)
			songs.forEach(song => {
				var directory = "https://taiko-asset.kairun.jp/songs/" + song.id + "/"
				song.music = new RemoteFile(directory + "main.ogg" + `?${ts}`)
				song.chart = new RemoteFile(directory + "main.tja" + `?${ts}`)
				if (song.title === "Nesin Amatias") {
					song.preview = 0
					song.previewMusic = new RemoteFile("https://taiko-asset.kairun.jp/preview/preview_tor.ogg" + `?${ts}`)
				} else if (song.title === "シン・ゾンビ") {
					song.preview = 0
					song.previewMusic = new RemoteFile("https://taiko-asset.kairun.jp/preview/preview_glyzmb.ogg" + `?${ts}`)
				}
	...
	load(){
		...
		this.addEdits(
			...
			new EditFunction(Search.prototype, "display").load(str => {
				str = plugins.insertBefore(str,
				`if(["ja", "cn", "tw", "ko"].includes(strings.id)){
					...
					border-radius: 0.5em;
					padding: 1em;\`
					msg.innerHTML = 'なにか質問あったらDiscordまでご報告ください！'
					bar.appendChild(msg)
				}
				`,
				`this.container =`)
			...
			new EditFunction(SongSelect.prototype, "toSelectDifficulty").load(str => {
				...
				}else if(currentSong.action === "ese_makers"){
					this.playSound("se_don")
					setTimeout(() => {
						window.open('/makers', '_blank');
					}, 500)
				`)
				return str
			}),
