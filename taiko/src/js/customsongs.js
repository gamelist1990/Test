class CustomSongs {
    ...
    init(touchEnabled, noPage, noLoading) {
        ...
        this.linkLocalZIP = document.getElementById("link-localzip")
        ...
        this.browseZIP = document.getElementById("browseZIP")
        pageEvents.add(this.browseZIP, "change", this.browseZIPChange.bind(this))
        this.setAltText(this.linkLocalZIP, strings.customSongs.localZIP)
        pageEvents.add(this.linkLocalZIP, ["mousedown", "touchstart"], this.localZIP.bind(this))
        this.items.push(this.linkLocalZIP)
        if (this.hasLocal) {
    ...
    localZIP(event) {
        ...
        this.changeSelected(this.linkLocalZIP)
        if (typeof showOpenFilePicker === "function" && !(/\bOPR\/|\bOPRGX\//.test(navigator.userAgent))) {
            return showOpenFilePicker({
                types: [
                    {
                        description: strings.customSongs.ZIPDescription,
                        accept: {
                            'application/zip': ['.zip']
                        }
                    },
                ...
                /** @arg {[FileSystemFileHandle]} files */
                async files => {
                    let filelist = [];
                    for (let i = 0; i < files.length; i++) {
                        filelist.push(await files[i].getFile());
                    }
                    this.importLocalZIP(filelist);
                }
            )
        } else {
            this.browseZIP.click()
        }
    }
    browseChange(event) {
        var files = []
    ...
    browseZIPChange(event) {
        let zipfiles = [];
        for (let i = 0; i < event.target.files.length; i++) {
            let item = event.target.files.item(i);
            zipfiles.push(item);
        }
        this.importLocalZIP(zipfiles);
    }
        ...
        try {
            await Promise.all(files.map((file) => new Promise(async (resolve) => {
                ...
                for (const filename of Object.keys(zip.files)) {
                    let fileData = await zip.files[filename].async('blob');
                    let file = new File([fileData], filename);
                    songs.push(new LocalFile(file, filename));
                }
                resolve();
            })));
        } catch {
            this.loading(false)
            this.locked = false;
            this.showError(strings.customSongs.errorZIP);
            return;
        }
        this.importLocal(songs);
    }
    gdriveFolder(event) {
        if (event) {
            if (event.type === "touchstart") {
